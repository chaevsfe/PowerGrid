...
public abstract class SegmentedBehaviour<T extends SegmentedBehaviour<T>> extends BlockEntityBehaviour {
    ...
    public void initialize() {
        if((!getLevel().isClientSide() || blockEntity.isVirtual()) && checkSizeConstraint()) {
            makeController();
    ...
    public void read(ValueInput compound, boolean clientPacket) {
        ...
        compound.read("LastKnownPos", BlockPos.CODEC).ifPresent(pos -> lastKnownPos = pos);
        boolean rebuild = compound.getBooleanOr("SegmentedRebuild", false) || segments == null;
        if(clientPacket && !rebuild)
            return;
        var storedController = compound.read("Controller", BlockPos.CODEC);
        if(storedController.isPresent()) {
            controllerPos = storedController.get();
            segments = null;
        } else {
            controllerPos = null;
            if(segments == null)
            ...
            if(clientPacket) {
                makeController();
                var level = getLevel();
                for(var pos : compound.listOrEmpty("Segments", BlockPos.CODEC)) {
    ...
    public void write(ValueOutput compound, boolean clientPacket) {
        ...
        if(clientPacket) {
            ...
            if(segments != null && isController()) {
                var segments = compound.list("Segments", BlockPos.CODEC);
                for (var segment : this.segments) {
                    segments.add(segment.getPos());
                }
            }
        }
        if (!isController()) {
