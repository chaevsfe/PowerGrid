...
public class BakedCircuit {
    ...
    public void write(ValueOutput tag) {
        ...
        tag.store("Thermal", CompoundTag.CODEC, thermalTag);
    }

    public void read(HolderLookup.Provider registries, ValueInput tag, boolean clientPacket) {
        var thermalOpt = tag.read("Thermal", CompoundTag.CODEC);
        if(thermalOpt.isPresent()) {
            var thermalTag = thermalOpt.get();
            for(var unit : thermalUnits) {
        ...
        if(!clientPacket)
            return;
        var schematic = tag.read("Schematic", CompoundTag.CODEC).orElse(null);
        if(schematic != null && schematic.contains("Components")) {
            for(var generic : schematic.getListOrEmpty("Components")) {
                var compound = (CompoundTag) generic;
                var id = compound.read("UUID", UUIDUtil.CODEC).orElse(null);
                for(var placed : tickedComponents) {
                    if(!placed.uuid.equals(id))
                        continue;
                    var changed = false;
                    var tagProperties = compound.getCompoundOrEmpty("Properties");
