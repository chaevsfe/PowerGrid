...
public class ElectronTubeComponent extends MirrorableComponent implements IRenderedComponent {
    ...
    public void dataFixup(@NotNull CompoundTag tag, int version) {
        if(version == 1) {
            var props = tag.getCompoundOrEmpty("Properties");
            ...
            if (fromAnodeResistance) {
                var resistance = props.getFloatOr("powergrid:tube_anode_resistance", 0.0f);
                var kg = ElectronTubeWire.calculateKg1(
                        1, 0,
                        props.getFloatOr(GAIN.id().toString(), 0.0f),
                        K_P.defaultValue(),
                        K_VB.defaultValue(),
                        EX.defaultValue(),
                        1 / resistance);
                ...
                var kg = props.getFloatOr(K_G.id().toString(), 0.0f);
