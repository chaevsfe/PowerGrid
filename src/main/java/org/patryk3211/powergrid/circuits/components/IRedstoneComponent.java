...
public interface IRedstoneComponent {
    ...
    static void notifyNeighbours(@NotNull PlacedComponent component) {
        ...
        if(component.has(Orientation.PROPERTY)) {
            var updateDir = CircuitBoardBlock.getDirection(state, component.get(Orientation.PROPERTY));
            var updatePos = component.getPos().relative(updateDir);
            component.getWorld().neighborChanged(updatePos, circuit, ExperimentalRedstoneUtils.initialOrientation(component.getWorld(), updateDir, null));
        } else {
            component.getWorld().updateNeighborsAt(component.getPos(), circuit, null);
        }
    }
}
