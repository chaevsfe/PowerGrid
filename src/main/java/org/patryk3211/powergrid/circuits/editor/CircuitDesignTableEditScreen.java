...
public class CircuitDesignTableEditScreen<T extends CircuitEditMenu<?>> extends AbstractSimiContainerScreen<T> {
    ...
    private CircuitEditWidget.SelectionResult placeTrace(int x1, int y1, int x2, int y2, int clickX, int clickY) {
        ...
        changed = true;
        playSound(ModdedSoundEvents.UI_PLACE_TRACE);
        if((schematic.isPad(clickX, clickY) || isTrace) ^ Minecraft.getInstance().hasShiftDown()) // Invert (XOR) if shifting
            return CircuitEditWidget.SelectionResult.BEGIN_NEW;
    ...
    public void extractBackground(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float partialTick) {
        ...
        for(int k = 0; k < this.menu.slots.size(); ++k) {
            ...
            if(slot == selectedSlot) {
                ctx.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND, leftPos + slot.x - 1, topPos + slot.y - 1, 232, 18, 18, 18, TEXTURE_SIZE, TEXTURE_SIZE);
                continue;
            }
            if(ComponentRegistry.getComponent(slot.getItem().getItem()) == null)
                continue;
            ctx.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND, leftPos + slot.x - 1, topPos + slot.y - 1, 232, 0, 18, 18, TEXTURE_SIZE, TEXTURE_SIZE);
        }
        ...
        if(currentTool.y > 0) {
            ctx.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND, leftPos + 173 - 11, topPos + currentTool.y, 250, 0, 6, 18, TEXTURE_SIZE, TEXTURE_SIZE);
        }
        ...
        if(unsavedPopupTimeout > 0) {
            ...
            color |= alpha << 24;
            ctx.centeredText(font, TEXT_NOT_SAVED, width / 2, topPos - 12, color);
        }
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        var handled = focused != null && focused.keyPressed(event);
        if(handled)
            return true;
        ...
        // Other tool keybinds
        if(ModdedKeys.ROTATE_COMPONENT.matches(event)) {
            if(currentComponent != null) {
                if(currentComponent.component.rotate(currentComponent, Minecraft.getInstance().hasShiftDown())) {
...
//            }
        } else if(ModdedKeys.PLACE_TRACE.matches(event)) {
            toolSelect(Tool.CONNECT);
            playSound(ModdedSoundEvents.UI_PLACE_TRACE);
        } else if(ModdedKeys.DELETE_AREA.matches(event)) {
            toolSelect(Tool.DELETE);
            playSound(ModdedSoundEvents.UI_DELETE_AREA);
        } else if(ModdedKeys.PICK_COMPONENT.matches(event)) {
            toolSelect(Tool.SELECT);
            playSound(ModdedSoundEvents.UI_SELECT_COMPONENT);
        } else if(ModdedKeys.SWITCH_LAYER.matches(event)) {
            flipLayer();
    ...
    public void extractRenderState(@NotNull GuiGraphicsExtractor ctx, int mouseX, int mouseY, float partialTicks) {
        ...
        if(saving) {
            ...
            ctx.centeredText(font, TEXT_SAVING, width / 2, height / 2 + 12, -1);
        }
    }

    @Override
    protected void slotClicked(Slot slot, int slotId, int button, ContainerInput actionType) {
        if(slot == null || !slot.hasItem() || actionType != ContainerInput.PICKUP)
            return;
        toolSelect(slot);
    }
    ...
    public static CircuitDesignTableEditScreen<CircuitDesignTableEditMenu> createTable(
        Minecraft minecraft, com.zurrtum.create.foundation.gui.menu.MenuType<CircuitDesignTableBlockEntity> type,
        int syncId, Inventory inventory, net.minecraft.network.chat.Component title, RegistryFriendlyByteBuf extraData
    ) {
        ...
        return type.create(CircuitDesignTableEditScreen<CircuitDesignTableEditMenu>::new, syncId, inventory, title, be);
    }

    public static CircuitDesignTableEditScreen<CircuitBoardEditMenu> createBoard(
        Minecraft minecraft, com.zurrtum.create.foundation.gui.menu.MenuType<CircuitBoardBlockEntity> type,
        int syncId, Inventory inventory, net.minecraft.network.chat.Component title, RegistryFriendlyByteBuf extraData
    ) {
        CircuitBoardBlockEntity be = getBlockEntity(minecraft, extraData);
        if(be == null)
            return null;
        try(var reporter = new ProblemReporter.ScopedCollector(be.problemPath(), PowerGrid.LOGGER)) {
            be.readClient(TagValueInput.create(reporter, extraData.registryAccess(), extraData.readNbt()));
        }
        return type.create(CircuitDesignTableEditScreen<CircuitBoardEditMenu>::new, syncId, inventory, title, be);
    }
