...
public class ComponentPropertiesWidget extends AbstractSimiWidget {
    ...
    private static void blitRepeating(
        ...
        for (int sliceWidth = Math.min(sourceWidth, width);
            ...
            for (int sliceHeight = Math.min(sourceHeight, height);
                ...
                ctx.blit(RenderPipelines.GUI_TEXTURED, texture, drawX, drawY, u, v, w, h, TEXTURE_SIZE, TEXTURE_SIZE);
            }
        }
    }
    ...
    protected void doRender(@NotNull GuiGraphicsExtractor ctx, int mouseX, int mouseY, float partialTicks) {
        ...
        var ms = ctx.pose();
        ms.translate(getX(), getY());

        // Three sliced texture
        int centerSliceSize = Math.max(width - 120, 0);
        ctx.blit(RenderPipelines.GUI_TEXTURED, PROPERTIES, 0, 0, 0, 0, 60, 16, TEXTURE_SIZE, TEXTURE_SIZE);
        if(width > 120) {
            blitRepeating(ctx, PROPERTIES, 60, 0, centerSliceSize, 16, 61, 0, 30, 16);
        }
        ctx.blit(RenderPipelines.GUI_TEXTURED, PROPERTIES, 60 + centerSliceSize, 0, 92, 0, 60, 16, TEXTURE_SIZE, TEXTURE_SIZE);

        var key = ComponentRegistry.getItem(component.component).getDescriptionId();
        ctx.centeredText(textRenderer, Component.translatable(key), getWidth() / 2, 5, 0xFFFFFFFF);
        ...
        for(var widget : propertyWidgets) {
            ...
            var label = Component.translatable(propertyKey);
            ctx.text(textRenderer, label, getX() + 6, widget.getY() + 6, 0xFF606060, false);
            int end = getX() + 6 + textRenderer.width(label);
            ...
            if(!summary.getString().isEmpty()) {
                ...
                if(mouseX >= x1 && mouseY >= y1 && mouseX < end && mouseY < y1 + 16)
                    ctx.setTooltipForNextFrame(textRenderer, lines, mouseX, mouseY);
            }
        }
        ms.popMatrix();

        ms.translate(0, yOffset);
        ctx.blit(RenderPipelines.GUI_TEXTURED, PROPERTIES, 0, 0, 0, 50, 60, 6, TEXTURE_SIZE, TEXTURE_SIZE);
        if(width > 120) {
            blitRepeating(ctx, PROPERTIES, 60, 0, centerSliceSize, 6, 61, 50, 30, 6);
        }
        ctx.blit(RenderPipelines.GUI_TEXTURED, PROPERTIES, 60 + centerSliceSize, 0, 92, 50, 60, 6, TEXTURE_SIZE, TEXTURE_SIZE);

        var stack = component.footprint().getRenderedStack(component.component);
        if(stack != null) {
            ctx.blit(RenderPipelines.GUI_TEXTURED, PROPERTIES, getWidth() / 2 - 8, 4, 240, 0, 16, 10, TEXTURE_SIZE, TEXTURE_SIZE);

            ms.pushMatrix();
            ms.translate(getWidth() / 2f, 14);
            ms.scale(3, 3);
            ms.translate(-8, 0);
            ctx.item(stack, 0, 0);
            ms.popMatrix();
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        ...
        for(var widget : propertyWidgets) {
            result |= widget.mouseClicked(event, doubled);
        }
        return result;
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        boolean result = false;
        for(var widget : propertyWidgets) {
            result |= widget.keyPressed(event);
        }
        return result;
    }
