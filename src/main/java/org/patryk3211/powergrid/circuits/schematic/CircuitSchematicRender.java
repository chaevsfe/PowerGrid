...
public class CircuitSchematicRender {
    ...
    public static void renderLayer(CircuitLayer layer, GuiGraphicsExtractor ctx, int xOffset, int yOffset, int scale, int color) {
    ...
    private static void drawTrace(int x1, int y1, int x2, int y2, GuiGraphicsExtractor ctx, int xOffset, int yOffset, int scale, int color) {
        ctx.fill(
    ...
    public static void renderPoints(List<Point> points, GuiGraphicsExtractor ctx, int x, int y, int scale, int color) {
    ...
    public static void renderComponents(CircuitSchematic schematic, GuiGraphicsExtractor ctx, int x, int y, int scale) {
        var ms = ctx.pose();
        ms.pushMatrix();
        ms.translate(x, y);
        ms.scale(scale, scale);
        for(var placed : schematic.components()) {
            FootprintRenderer.render(placed.footprint(), ctx, placed.component, placed.x * GRID_TO_GRID_SCALE, placed.y * GRID_TO_GRID_SCALE, false);
        }
        ms.popMatrix();
    }

    public static void renderComponents(CircuitSchematic schematic, GuiGraphicsExtractor ctx, int x, int y, int scale, int mouseX, int mouseY) {
        var ms = ctx.pose();
        ms.pushMatrix();
        ms.translate(x, y);
        ms.scale(scale, scale);
        ...
        for(var placed : schematic.components()) {
            ...
            FootprintRenderer.render(placed.footprint(), ctx, placed.component, placed.x * GRID_TO_GRID_SCALE, placed.y * GRID_TO_GRID_SCALE, hovering);
        }
        ms.popMatrix();
    }
}
