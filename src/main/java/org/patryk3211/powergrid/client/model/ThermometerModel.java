...
public class ThermometerModel implements ItemModel {
    public static final Identifier ID = PowerGrid.asResource("model/thermometer");
    public static final Identifier ITEM_ID = PowerGrid.asResource("item/thermometer");
    public static final Identifier NEEDLE_ID = PowerGrid.asResource("block/thermometer/needle");

    private static final float NEEDLE_PIVOT = 0.5f - 2f / 16f;
    ...
    public ThermometerModel(List<BakedQuad> itemQuads, ModelRenderProperties settings, List<BakedQuad> needleQuads) {
    ...
    public void update(ItemStackRenderState state, ItemStack stack, ItemModelResolver resolver, ItemDisplayContext transformType, ClientLevel level, ItemOwner owner, int seed) {
        ...
        var angle = ThermometerItemRenderer.needleAngle(AnimationTickHolder.getPartialTicks());
        needleLayer.setLocalTransform(new Matrix4f().rotateAround(new Quaternionf().rotateZ(angle),
    ...
    public static class Unbaked implements ItemModel.Unbaked {
        ...
        public ItemModel bake(ItemModel.BakingContext context, Matrix4fc transform) {
            ...
            return new ThermometerModel(parts.quads(), parts.properties(), ItemModelParts.quads(baker, NEEDLE_ID));
        }
    }
}
