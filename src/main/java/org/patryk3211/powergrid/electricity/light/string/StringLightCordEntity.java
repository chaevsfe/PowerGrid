...
public class StringLightCordEntity extends CordEntity {
    ...
    public static StringLightCordEntity create(Level world, ICordEndpoint endpoint1, ICordEndpoint endpoint2, ItemStack item, @Nullable Float resistance) {
        ...
        var pattern = item.get(ModdedDataComponents.LIGHT_PATTERN);
        if(pattern != null){
    ...
    protected void readAdditionalSaveData(ValueInput nbt) {
        ...
        filamentTemperature = nbt.getFloatOr("Filament", 0.0f);
        if(nbt.contains("Pattern")) {
            colorPattern = nbt.getIntArray("Pattern").orElse(new int[0]);
    ...
    @Override
    public void tick() {
        if(!level().isClientSide() || LevelKind.isPonder(level())) {
