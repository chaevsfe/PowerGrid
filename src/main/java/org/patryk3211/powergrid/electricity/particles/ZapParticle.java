...
import org.patryk3211.powergrid.compat.sable.ClientSubLevelAccess;
...
public class ZapParticle extends Particle {
    ...
    private final float rCol = 1;
    private final float gCol = 1;
    private final float bCol = 0.5f;
    private final float alpha = 1;

    private final List<Pair<Vec3, Vec3>> segments = new ArrayList<>();
    ...
    public void addSegment(Vec3 pos1, Vec3 pos2) {
        segments.add(Pair.of(pos1, pos2));
    }

    public void addSegment(double x1, double y1, double z1, double x2, double y2, double z2) {
        segments.add(Pair.of(new Vec3(x1, y1, z1), new Vec3(x2, y2, z2)));
    }
    ...
    public void extract(ZapParticleRenderState state, Vec3 camPos, float tickDelta) {
        int argbColor = ((int) (alpha * 255) << 24) | ((int) (rCol * 255) << 16) | ((int) (gCol * 255) << 8) | (int) (bCol * 255);

        var pose = sublevel != null ? sublevel.renderPose(tickDelta) : null;
        for(var segment : segments) {
            var a = segment.getFirst();
            var b = segment.getSecond();
            if(pose != null) {
    ...
    public static void renderSegment(VertexConsumer buffer, double x1, double y1, double z1, double x2, double y2, double z2, Vec3 cross1, Vec3 cross2, int color) {
    ...
    public ParticleRenderType getGroup() {
        return ZapParticleGroup.ZAP;
    }

    @Environment(EnvType.CLIENT)
    public static class Factory implements ParticleProvider<ZapParticleData> {
        @Override
        public Particle createParticle(ZapParticleData data, ClientLevel world, double x, double y, double z, double velocityX, double velocityY, double velocityZ, RandomSource random) {
            return new ZapParticle(data, world, x, y, z, velocityX, velocityY, velocityZ);
        }
    }
}
