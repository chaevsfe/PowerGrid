...
public class SolarHelper {
    ...
    public static double getAM(Level world, BlockPos pos){
        var sunAngle = Math.max(0, Math.cos(getSunAngle(world, pos)));
        if (sunAngle <= 0) {
    ...
    public static boolean skyCheck(Level world, BlockPos pos) {
        ...
        if (subLevel == null) {
            if (!world.canSeeSky(pos)) {
                ...
                var list = DDA(world, Vec3.atCenterOf(pos), Vec3.atCenterOf(pos).add(0, topY - pos.getY(), 0));
                boolean hit = false;
            ...
            if (topYSubLevel > pos.getY()) {
                var list = DDA(world, Vec3.atCenterOf(pos), Vec3.atCenterOf(pos).add(0, topYSubLevel - pos.getY(), 0));
                for (DDAHit result : list) {
                    BlockState blockState;
                    if (result.contraption() != null) {
                        blockState = result.contraption().getContraption().getBlocks().get(result.worldOrLocalPos()).state();
            ...
            if (topYWorld > worldPos.getY()) {
                var list = DDA(world, Vec3.atCenterOf(worldPos), Vec3.atCenterOf(worldPos).add(0, topYWorld - worldPos.getY(), 0));
                for (DDAHit result : list) {
                    BlockState blockState;
                    if (result.contraption() != null) {
                        blockState = result.contraption().getContraption().getBlocks().get(result.worldOrLocalPos()).state();
