...
public abstract class BaseWireEntity extends Entity implements EntityDataS2CPacket.IConsumer {
    ...
    private void temperatureUpdate() {
        ...
        if(LevelKind.isRealClient(level()))
            return;
    ...
    public void onEntityDataPacket(CompoundTag data) {
        int version = data.getIntOr("Version", 0);
        if(version < dataVersion) {
            // Discard outdated packet.
            return;
        }
        readAdditionalSaveData(TagValueInput.create(ProblemReporter.DISCARDING, registryAccess(), data));
        dataVersion = version + 1;
    }

    @Override
    protected void readAdditionalSaveData(ValueInput nbt) {
        var storedItem = nbt.child("Item");
        if(storedItem.isPresent()) {
            var itemTag = storedItem.get();
            var readItem = BuiltInRegistries.ITEM.getValue(Identifier.parse(itemTag.getStringOr("Id", "")));
            if(!IWire.isWire(level(), readItem))
                throw new IllegalStateException("WireEntity item must be a WireItem");
            setItem(readItem, itemTag.getIntOr("Count", 0));
        ...
        BlockPos lastPos = nbt.read("LastKnownPos", BlockPos.CODEC).orElse(null);

        IWireEndpoint endpoint1 = WireEndpointType.deserialize(nbt.read("Endpoint1", CompoundTag.CODEC).orElse(null));
        IWireEndpoint endpoint2 = WireEndpointType.deserialize(nbt.read("Endpoint2", CompoundTag.CODEC).orElse(null));
        ...
        entityData.set(TEMPERATURE, nbt.getFloatOr("Temperature", 0.0f));
    }
    ...
    protected void addAdditionalSaveData(ValueOutput nbt) {
        if(endpoint1 != null)
            nbt.store("Endpoint1", CompoundTag.CODEC, endpoint1.serialize());

        if(endpoint2 != null)
            nbt.store("Endpoint2", CompoundTag.CODEC, endpoint2.serialize());

        var itemTag = nbt.child("Item");
        itemTag.putString("Id", BuiltInRegistries.ITEM.getKey(item).toString());
        itemTag.putInt("Count", itemCount);
        if(wireEntry.colorable())
            nbt.putInt("Color", color);

        nbt.store("LastKnownPos", BlockPos.CODEC, blockPosition());
    ...
    public void kill(ServerLevel level) {
        for(int i = itemCount; i > 0; i -= 64) {
            spawnAtLocation(level, new ItemStack(item, Math.min(i, 64)));
        }
        itemCount = 0;
        super.kill(level);
    }
    ...
    public InteractionResult interact(Player player, InteractionHand hand, Vec3 hitPos) {
        ...
        if(stack.is(ModdedTags.Item.WIRE_CUTTERS.tag)) {
            ...
            if(wireEntry.colorable() && dyeColor != null) {
                setColor(dyeColor);
                if(!level().isClientSide()) {
                    sendExtraData();
                }
