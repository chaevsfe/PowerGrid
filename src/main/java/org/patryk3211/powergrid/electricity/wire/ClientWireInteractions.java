...
public class ClientWireInteractions {
    ...
    public static void clientTick() {
        ...
        if(segment != null) {
            ...
            if(segment.getFirst() < firstSegmentIndex) {
                index1 = segment.getFirst();
                point1 = segment.getSecond();
                index2 = firstSegmentIndex;
                point2 = firstSegmentPoint;
            } else {
                index1 = firstSegmentIndex;
                index2 = segment.getFirst();
                if(index1 == index2 && segment.getSecond() < firstSegmentPoint) {
                    point1 = segment.getSecond();
                    point2 = firstSegmentPoint;
                } else {
                    point1 = firstSegmentPoint;
                    point2 = segment.getSecond();
                }
            }
            ...
            for(int i = index1; i <= index2; ++i) {
                ...
                mc.level.addAlwaysVisibleParticle(new DustParticleOptions(0xFF8080, 0.5f),
                        pos.x, pos.y, pos.z, 0, 0, 0);
            }
        }
    }

    private static Pair<Integer, Integer> getSegment(BlockWireEntity entity, Vec3 hitPos) {
    ...
    public static InteractionResult segmentCut(BlockWireEntity entity) {
        ...
        if(currentEntity != entity) {
            ...
            if(segment != null) {
                firstSegmentIndex = segment.getFirst();
                firstSegmentPoint = segment.getSecond();
            ...
            if(secondSegment != null) {
                ModdedPackets.sendToServer(new BlockWireCutC2SPacket(entity, firstSegmentIndex, firstSegmentPoint, secondSegment.getFirst(), secondSegment.getSecond()));
                currentEntity = null;
    ...
    public static boolean cutClearCheck(Minecraft client) {
        ...
        if(currentEntity != null) {
            client.player.sendOverlayMessage(Lang.translateDirect("message.cut_reset"));
    ...
    public static InteractionResult attachWire(BlockWireEntity entity) {
        ...
        if(entity.getItem() != stack.getItem()) {
            mc.player.sendOverlayMessage(Lang.translate("message.connection_incorrect_wire_type").style(ChatFormatting.RED).component());
            return InteractionResult.FAIL;
        }
        ...
        if(existingEndpoint != null && existingEndpoint.getSubLevel(mc.level) != SableCompanion.INSTANCE.getContaining(entity)) {
            mc.player.sendOverlayMessage(Lang.translate("message.connection_failed").style(ChatFormatting.RED).component());
            return InteractionResult.FAIL;
        }
        ...
        ModdedPackets.sendToServer(new BlockWireAttachC2SPacket(entity, segment.getFirst(), segment.getSecond()));
        return InteractionResult.SUCCESS;
    }

    public static void alternatePlacementCheck(Minecraft client, int action) {
        var stack = client.player.getMainHandItem();
        if(!IWire.isWire(client.level, stack.getItem()) || !stack.has(ModdedDataComponents.CONNECTION_DATA)) {
