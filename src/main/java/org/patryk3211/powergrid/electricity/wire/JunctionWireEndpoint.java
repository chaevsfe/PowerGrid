...
public class JunctionWireEndpoint implements IWireEndpoint {
    private static final Map<Level, WorldEntry> JUNCTION_NODES = new ConcurrentHashMap<>();
    ...
    public void read(CompoundTag nbt) {
        pos = new Vec3(
                nbt.getFloatOr("X", 0.0f),
                nbt.getFloatOr("Y", 0.0f),
                nbt.getFloatOr("Z", 0.0f)
        );
        id = nbt.read("Id", UUIDUtil.CODEC).orElse(null);
    }
