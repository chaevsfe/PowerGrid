...
public class WireConnection {
    ...
    public static WireConnection of(BlockPos pos, int nTurns, BlockWireEndpoint blockEndpoint) {
        ...
        trTag.store("Initiator", BlockPos.CODEC, pos);
    ...
    public IWireEndpoint endpoint() {
        return WireEndpointType.deserialize(tag.getCompoundOrEmpty("Connection"));
    }
    ...
    public IWireEndpoint half() {
        return WireEndpointType.deserialize(tag.getCompoundOrEmpty("Half"));
    }
    ...
    public BlockPos getTransformerInitiator() {
        var trTag = tag.getCompoundOrEmpty("Transformer");
        return trTag.read("Initiator", BlockPos.CODEC).orElse(null);
    }

    public int getTransformerTurns() {
        var trTag = tag.getCompoundOrEmpty("Transformer");
        return trTag.getIntOr("Turns", 0);
    }

    public int getTransformerTerminal() {
        var trTag = tag.getCompoundOrEmpty("Transformer");
        return trTag.getIntOr("Terminal", 0);
    }

    public WireConnection withTurns(int nTurns) {
        var result = new WireConnection(tag.copy());
        result.tag.getCompoundOrEmpty("Transformer").putInt("Turns", nTurns);
        return result;
    }
