...
public class AutoCordEndpoint implements ICordEndpoint {
    ...
    public void read(CompoundTag nbt) {
        pos = nbt.read("Position", BlockPos.CODEC).orElseThrow();
        terminal1 = nbt.getIntOr("Terminal1", 0);
        terminal2 = nbt.getIntOr("Terminal2", 0);
        placement = new Vec3(nbt.getDoubleOr("X", 0.0), nbt.getDoubleOr("Y", 0.0), nbt.getDoubleOr("Z", 0.0));
        if(nbt.contains("Plug")) {
            plugFacing = Direction.values()[Mth.clamp(nbt.getByteOr("Plug", (byte) 0), 0, Direction.values().length - 1)];
        } else {
            plugFacing = null;
        }
    }
    ...
    public void write(CompoundTag nbt) {
        nbt.store("Position", BlockPos.CODEC, pos);
