...
public class CordEntity extends BaseWireEntity implements IComplexRaycast {
    ...
    public void refreshTerminalPositions() {
        ...
        if(world != null && (!world.isClientSide() || LevelKind.isPonder(world))) {
    ...
    public void onEntityDataPacket(CompoundTag data) {
        if(data.contains("V")) {
            var list = data.getListOrEmpty("V");
            terminalPos1 = new Vec3(list.getDoubleOr(0, 0.0), list.getDoubleOr(1, 0.0), list.getDoubleOr(2, 0.0));
            terminalPos2 = new Vec3(list.getDoubleOr(3, 0.0), list.getDoubleOr(4, 0.0), list.getDoubleOr(5, 0.0));
            dynamic = data.getByteOr("D", (byte) 0);
            updateCurveParams();
        } else {
    ...
    public InteractionResult interact(Player player, InteractionHand hand, Vec3 hitPos) {
        InteractionResult result = super.interact(player, hand, hitPos);
    ...
    public boolean cordDetach(Player player, boolean secondEndpoint) {
        ...
        stack.set(ModdedDataComponents.CONNECTION_DATA, WireConnection.of(secondEndpoint ? endpoint1 : endpoint2));
    ...
    protected void readAdditionalSaveData(ValueInput nbt) {
        ...
        dynamic = nbt.getByteOr("Dynamic", (byte) 0);
        var world = level();
        if(!world.isClientSide()) {
        ...
        if(nbt.contains("PlacedLength")) {
            placedLength = nbt.getFloatOr("PlacedLength", 0.0f);
        } else {
