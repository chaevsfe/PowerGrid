...
public class CordItem extends WireItem {
    ...
    private static InteractionResult addEndpoint(UseOnContext context, ICordEndpoint endpoint) {
        var stack = context.getItemInHand();
        var firstPoint = stack.getOrDefault(ModdedDataComponents.CONNECTION_DATA, WireConnection.EMPTY).endpoint();
        if(firstPoint == null) {
    ...
    public static InteractionResult useOn(Player player, InteractionHand hand, BlockPos blockPos, Direction direction) {
        ...
        if(electric != null) {
            ...
            if(terminal >= 0) {
                var connection = stack.getOrDefault(ModdedDataComponents.CONNECTION_DATA, WireConnection.EMPTY);
...
//                    stack.set(DataComponents.CUSTOM_DATA, CustomData.of(compoundTag));
                    return (addEndpoint(context, splitEndpoint).consumesAction()) ? InteractionResult.SUCCESS : InteractionResult.FAIL;
                } else {
                    var endpoint = new BlockWireEndpoint(blockPos, terminal);
                    stack.set(ModdedDataComponents.CONNECTION_DATA, connection.withHalf(endpoint));
        ...
        for(var handler : PLACEMENT_HANDLERS) {
            ...
            if(result.getResult().consumesAction())
                return (addEndpoint(context, result.getObject()).consumesAction()) ? InteractionResult.SUCCESS : InteractionResult.FAIL;
