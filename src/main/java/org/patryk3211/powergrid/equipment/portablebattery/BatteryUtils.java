...
public class BatteryUtils {
    ...
    public static int getCurrentCharge(ItemStack stack) {
        return stack.getOrDefault(ModdedDataComponents.PORTABLE_BATTERY_CHARGE, 0);
    }
    ...
    public static float drawEnergy(Player player, int energy) {
        ...
        if(charge < energy) {
            stack.remove(ModdedDataComponents.PORTABLE_BATTERY_CHARGE);
            return 0.0f;
        }
        stack.set(ModdedDataComponents.PORTABLE_BATTERY_CHARGE, Math.max(charge - energy, 0));
    ...
    public static float tryDrawEnergy(ItemStack battery, int energy) {
        ...
        if(energy == 0 || !battery.has(ModdedDataComponents.PORTABLE_BATTERY_CHARGE))
            return 0.0f;
        if(charge < energy)
            return 0.0f;
        return outputPercent;
    ...
    public static boolean isBarVisible(ItemStack stack, int energyPerUse, float minPower) {
        if(energyPerUse == 0)
            return false;
        return Env.CLIENT.getIfCurrent(() -> ClientSideAccess::player)
                .map(player -> {
