...
public class CeilingTileBlock extends Block implements CeilingBlock {
    ...
    protected InteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        ...
        if (stack.is(ModdedBlocks.FACTORY_LIGHT.asItem())) {
            ...
            if(!level.isClientSide()) {
                ...
                level.playSound(null, pos,
                        ...
                        SoundSource.BLOCKS, 1.0f, level.getRandom().nextFloat() * 0.25f + 1.0f);
            }
            return InteractionResult.SUCCESS;
        }
        if (stack.is(ModdedBlocks.WIRE_CONNECTOR.asItem())) {
            if(!player.isCreative())
                stack.shrink(1);
            if(!level.isClientSide()) {
        ...
        if (stack.is(ModdedBlocks.CORD_JUNCTION.asItem())) {
            if(!player.isCreative())
                stack.shrink(1);
            if(!level.isClientSide()) {
        ...
        if (stack.is(ModdedBlocks.SOLAR_PANEL.asItem())) {
            if(!player.isCreative())
                stack.shrink(1);
            if(!level.isClientSide()) {
