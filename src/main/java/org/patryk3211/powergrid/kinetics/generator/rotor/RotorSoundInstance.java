...
public class RotorSoundInstance extends AbstractTickableSoundInstance {
    ...
    public RotorSoundInstance(RotorBehaviour behaviour) {
        super(ModdedSoundEvents.GENERATOR.getMainEvent(), SoundSource.AMBIENT, behaviour.getLevel().getRandom());
        this.behaviour = behaviour;
        var pos = Vec3.atCenterOf(behaviour.getPos());
        this.x = pos.x;
        this.y = pos.y;
        this.z = pos.z;
    ...
    public void tick() {
        if(behaviour.blockEntity.isRemoved() || !behaviour.isController()) {
            ...
            if(closest.getValue() != null) {
                var pos = Vec3.atCenterOf(closest.getValue());
                this.x = pos.x;
                this.y = pos.y;
                this.z = pos.z;
            }
