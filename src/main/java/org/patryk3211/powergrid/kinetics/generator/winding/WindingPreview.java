...
public class WindingPreview {
    ...
    public static ItemStack getUsedWireStack(Player player) {
        ...
        if(stack1 != null && stack1.getItem() instanceof WindingItem && stack1.has(ModdedDataComponents.WINDING_CONNECTION)) {
            return stack1;
        } else if(stack2 != null && stack2.getItem() instanceof WindingItem && stack2.has(ModdedDataComponents.WINDING_CONNECTION)) {
            return stack2;
        } else {
            return null;
        }
    ...
    public static void tick() {
        ...
        var firstPos = stack.get(ModdedDataComponents.WINDING_CONNECTION);
        ...
        if(!(rayTrace instanceof BlockHitResult hit)) {
            if(r.nextInt(50) == 0) {
                world.addParticle(new DustParticleOptions(0x4CE57F, 1),
       ...
       if (!ShaftBlock.isShaft(selectedState))
        ...
        for (float f = 0; f < Math.abs(length); f += .0625f) {
            ...
            if (r.nextInt(10) == 0) {
                world.addParticle(
                        new DustParticleOptions(canConnect ? 0x4CE57F : 0xE54C7F, 1),
