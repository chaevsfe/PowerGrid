...
public class PunchCardScreen extends AbstractSimiContainerScreen<PunchCardMenu> {
    public static final Identifier BACKGROUND = PowerGrid.texture("gui/punch_card_edit");
    ...
    public static PunchCardScreen create(
        Minecraft minecraft, MenuType<net.minecraft.world.item.ItemStack> type, int syncId, Inventory inventory, Component title,
        RegistryFriendlyByteBuf extraData
    ) {
        return type.create(PunchCardScreen::new, syncId, inventory, title, getStack(extraData));
    }
    ...
    protected void init() {
        ...
        if(!lock) {
            ...
            var author = new StringWidget(leftPos + 7, topPos + 109, font.width(Lang.translate("gui.punch_card.author")
                    .add(Component.literal(menu.getAuthor()))
                    .component()), 9, Lang.translate("gui.punch_card.author")
                    .add(Component.literal(menu.getAuthor()))
                    .component(), font);

            var nameWidget = new StringWidget(leftPos + 60, topPos + 4, font.width(name), 9, Component.literal(name.getString()), font);
    ...
    protected void renderForeground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
        for(var particle : particles) {
            particle.render(graphics, partialTicks);
        }
    ...
    private static class Particle {
        ...
        public void render(GuiGraphicsExtractor graphics, float partialTick) {
            ...
            pose.pushMatrix();
            pose.translate(x + vx * partialTick, y + vy * partialTick);
            pose.rotate(angle + va * partialTick);
            var s = Math.min(ttl / 40f, 1.0f);
            pose.scale(s, s);
            graphics.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND, -5, -7, 228, 1, 11, 15, TEXTURE_SIZE, TEXTURE_SIZE);
            pose.popMatrix();
        }
    }
}
