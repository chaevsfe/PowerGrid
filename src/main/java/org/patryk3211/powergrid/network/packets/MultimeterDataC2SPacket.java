...
public class MultimeterDataC2SPacket implements C2SPacket {
    ...
    public void handle(ServerPlayer player) {
        ...
        if(!(entity instanceof BaseWireEntity wireEntity))
            return;
        var box = wireEntity.getBoundingBox();
        if(box.distanceToSqr(player.position()) > C2SPacket.MAX_INTERACTION_DISTANCE_SQUARED)
            return;
        if(!box.inflate(1).contains(new Vec3(point.x, point.y, point.z)))
            return;
        ...
        data.store("UUID", UUIDUtil.CODEC, entity.getUUID());
