...
public class SaveSchematicC2SPacket implements C2SPacket {
    ...
    public void handle(ServerPlayer player) {
        ...
        if(be instanceof CircuitDesignTableBlockEntity table) {
            if(nbt != null) {
                table.getSchematic().deserializeNbt(player.level().registryAccess(), nbt);
                table.setSchematicName(name);
                table.notifyUpdate();

                // Saved successfully.
                IMultiScreenHandlerFactory.openScreen(player, table, 0);
            } else {
                if(load) {
            ...
            if(player.isCreative() && nbt != null) {
                holder.setSchematic(CircuitSchematic.fromNbt(player.level().registryAccess(), nbt));
            }
        }
    }
}
