...
public class PlacementOverlay {
    ...
    public static void renderOverlay(GuiGraphicsExtractor graphics) {
        ...
        var font = mc.font;
        int y = graphics.guiHeight() - 61;
        int alpha = Mth.clamp(Math.round(Mth.clamp(overlayTicks, 0, 10) / 10.0f * 255), 0, 255);
        int color = (alpha << 24) | (new Color(0x4adb4a).getRGB() & 0xFFFFFF);
        int i = 0;
        for(var text : lines) {
    ...
    public static Component getTransformerText(Player player) {
        ...
        var initiatorPos = tag.read("Initiator", BlockPos.CODEC).orElse(null);
        ...
        var turns = tag.getIntOr("Turns", 0);
